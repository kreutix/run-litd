...
BINARY_URL="https://github.com/lightninglabs/lightning-terminal/releases/download/$LITD_VERSION/lightning-terminal-linux-amd64.tar.gz"
...
if [[ -f "/usr/local/bin/litd" ]]; then
    ...
    echo "[+] Ensuring $BINARY_DIR directory exists..."
    mkdir -p "$BINARY_DIR"

    echo "[+] Downloading litd binary and related files..."
    curl -L $BINARY_URL -o "$BINARY_DIR/lightning-terminal.tar.gz"
    curl -L $SIGNATURE_URL -o "$BINARY_DIR/manifest-guggero-$LITD_VERSION.sig"
    curl -L $MANIFEST_URL -o "$BINARY_DIR/manifest-$LITD_VERSION.txt"

    echo "[+] Verifying PGP signature..."
    gpg --keyserver hkps://keyserver.ubuntu.com --recv-keys F4FC70F07310028424EFC20A8E4256593F177720
    gpg --verify "$BINARY_DIR/manifest-guggero-$LITD_VERSION.sig" "$BINARY_DIR/manifest-$LITD_VERSION.txt"

    echo "[+] Checking SHA256 checksum..."
    EXPECTED_CHECKSUM=$(grep "linux-amd64" "$BINARY_DIR/manifest-$LITD_VERSION.txt" | awk '{print $1}')
    CHECKSUM=$(shasum -a 256 "$BINARY_DIR/lightning-terminal.tar.gz" | awk '{print $1}')
    ...
    tar -xf "$BINARY_DIR/lightning-terminal.tar.gz" -C "$BINARY_DIR" --strip-components=1

    echo "[+] Moving binaries to /usr/local/bin..."
    sudo mv "$BINARY_DIR/litd" /usr/local/bin/litd
    sudo chmod +x /usr/local/bin/litd
